...
func (r *ProcessRunner) Start(ctx context.Context, opts Options) (*ProcessHandle, error) {
    ...
    if stdout != nil { go streamLogs(ctx, opts.Name, "stdout", stdout) }
    if stderr != nil { go streamLogs(ctx, opts.Name, "stderr", stderr) }
...
func (r *ProcessRunner) RunManaged(ctx context.Context, name string, opts Options, hc HealthConfig, policy RestartPolicy, onStart func(*ProcessHandle), onHealth func(bool)) error {
    if hc.Interval == 0 { hc.Interval = 10 * time.Second }
    if hc.Timeout == 0 { hc.Timeout = 3 * time.Second }
    if hc.FailureThreshold == 0 { hc.FailureThreshold = 3 }

    for {
        // Start
        handle, err := r.Start(ctx, opts)
        if err != nil { return err }
        if onStart != nil { onStart(handle) }
...
func probeOnce(hc HealthConfig, opts Options) bool {
    // Support http://, https://
    if len(opts.Env) > 0 { _ = opts.Env } // placeholder for future env substitution
    if u := hc.Check; u != "" {
        ...
        if hasPrefix(u, "cmd:") {
            ...
            if opts.WorkingDir != "" { cmd.Dir = opts.WorkingDir }
            return cmd.Run() == nil
        }
    }
    return true
