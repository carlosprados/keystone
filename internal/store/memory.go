...
func (s *MemoryStore) Upsert(ci ComponentInfo) {
    ...
    if ok {
        if ci.State == "" { ci.State = prev.State }
        if ci.Restarts == 0 { ci.Restarts = prev.Restarts }
        if ci.LastHealth == "" { ci.LastHealth = prev.LastHealth }
        if ci.PID == 0 { ci.PID = prev.PID }
    }
    if ci.LastHealth == "" { ci.LastHealth = "unknown" }
    s.items[ci.Name] = ci
    s.mu.Unlock()
}
