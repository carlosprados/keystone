...
func LoadTrustBundle(pemPath string) (*x509.CertPool, error) {
    if pemPath == "" { return nil, errors.New("empty trust bundle path") }
    b, err := os.ReadFile(pemPath)
    if err != nil { return nil, err }
    pool := x509.NewCertPool()
    if !pool.AppendCertsFromPEM(b) { return nil, errors.New("failed to parse trust bundle") }
    return pool, nil
}
...
func VerifyDetached(filePath, sigPath, leafCertPath string, roots *x509.CertPool) error {
    if roots == nil { return errors.New("nil trust roots") }
    ...
    if len(certs) == 0 { return errors.New("no certificate found") }
    leaf := certs[0]

    // Verify chain
    if _, err := leaf.Verify(x509.VerifyOptions{Roots: roots}); err != nil { return fmt.Errorf("certificate verify failed: %w", err) }
    ...
    case *ecdsa.PublicKey:
        ...
        if _, err := asn1.Unmarshal(sig, &esig); err != nil { return fmt.Errorf("ecdsa sig parse: %w", err) }
        if esig.R == nil || esig.S == nil { return errors.New("invalid ecdsa signature") }
        if !ecdsa.Verify(pub, digest, esig.R, esig.S) { return errors.New("ecdsa verify failed") }
    default:
...
func parsePEMCerts(b []byte) ([]*x509.Certificate, error) {
    ...
    for {
        ...
        if err == nil { out = append(out, c) }
    }
    if len(out) == 0 { return nil, errors.New("no certs in PEM") }
    return out, nil
}
