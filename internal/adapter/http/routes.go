...
func (a *Adapter) routes() []Route {
	return []Route{
		...
		},
		{
			...
			Description:   "What each refreshable dataset is currently serving: version, when it was published, when it was last refreshed, and how old it is. `stale` means it has passed its max_age. `ageUnknown` means the device's clock cannot be trusted, so no age is reported rather than a made-up one.",
			Response:      new([]adapter.DatasetInfo),
			SuccessStatus: http.StatusOK,
			handler:       a.handleDatasets,
		},
		{
			Pattern:       "/v1/datasets:refresh",
			Path:          "/v1/datasets:refresh",
			Methods:       []string{http.MethodPost},
			Summary:       "Refresh datasets now",
			Description:   "Checks every dataset for a new version immediately, instead of waiting for its refresh interval — for the day a vulnerability feed cannot wait until tonight. Each dataset still goes through signature verification and the anti-replay rule.",
			Response:      new([]adapter.DatasetInfo),
			SuccessStatus: http.StatusOK,
			Errors:        []int{http.StatusInternalServerError},
			handler:       a.handleDatasetsRefresh,
		},
		{
