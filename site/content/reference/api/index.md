...
The document below is **generated from the agent's route table** by
`cmd/openapi-gen`, so it describes the endpoints the binary actually serves. CI
regenerates it and fails if it differs from the checked-in copy — a renamed field in
a response type cannot reach `main` with the spec still claiming the old name.

{{% notice style="tip" title="Use it with your own tools" %}}
The raw document is published alongside this page:
[`openapi.yaml`](openapi.yaml). Point `oapi-codegen`, `openapi-generator` or your
HTTP client at it, or run `task openapi` in a checkout to regenerate it.
{{% /notice %}}

{{< openapi src="openapi.yaml" >}}

## The endpoints in prose

The generated reference above is exhaustive. What follows is the same surface with
the reasoning attached — why an endpoint behaves the way it does, and the traps.
