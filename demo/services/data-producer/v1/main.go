...
func main() {
	...
	log.Printf("[producer v%s] config loaded: rate_ms=%d batch=%d", version, cfg.RateMs, cfg.BatchSize)
...
func produce(ctx context.Context, buf *buffer, cfg remoteConfig) {
	...
	for {
		...
		case <-tick.C:
			...
			id := buf.nextID
			buf.mu.Unlock()
			e := event{ID: id, Greeting: cfg.Greeting, Timestamp: time.Now()}
			buf.push(e)
		}
	}
}
